...
public class AdminController {
	...
	public String addProductPost(@Valid Product product, BindingResult result) {
		...
		if(!productService.addProuct(product)) // false이면
			System.out.println("Adding product cannot be done");
		
		// return "productInventory"; //prodcutInventory.jsp에서 사용하는 products를
									  //model에 넣지 않아서 jsp에서 아무것도 출력해주지 않음
		return "redirect:/admin/productInventory"; // Controller에서 처리가 되어 productInventory.jsp로 연결됨
	}
	
	@RequestMapping(value="/productInventory/deleteProduct/{id}", method=RequestMethod.GET)
	public String deleteProduct(@PathVariable int id) {
		if(!productService.deleteProuct(id)) // false이면
	...
	public String updateProductPost(@Valid Product product, BindingResult result) {
		...
		if(!productService.updateProuct(product)) // false이면
